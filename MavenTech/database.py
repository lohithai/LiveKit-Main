...
MONGO_DB_NAME = os.getenv("MONGO_DB_NAME", "maventech")
CONTEXT_COLLECTION = os.getenv("MONGO_CONTEXT_COLLECTION", "maventech_context")
...
async def get_async_context_collection():
    """Get the MavenTech context collection."""
...
async def get_async_collection(collection_name: str):
    """Get any MongoDB collection by name (e.g. 'bookings', 'tool_logs')."""
    client = await get_async_client()
    db = client[MONGO_DB_NAME]
