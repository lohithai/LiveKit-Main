"""
MavenTech Bus Booking Voice AI Agent — LiveKit Entry Point
Sarvam STT + Gemini 2.5 Flash LLM + Cartesia TTS + Silero VAD
"""
...
from assistant import MavenTechAssistant
...
AGENT_NAME = os.getenv("AGENT_NAME", "maventech")
...
async def maventech_agent(ctx: agents.JobContext):
    """MavenTech voice agent — handles both inbound and outbound SIP calls."""
    ...
    assistant = MavenTechAssistant(
        voice_user_id=voice_user_id,
        user_id=user_id,
        user_contexts=user_contexts,
    )

    # ── 7. Build STT (Sarvam saaras:v3 — streaming + interim) ──────
    stt = sarvam.STT(
    ...
    if name and name not in ["", "Voice User", "Unknown"]:
        greeting = (
            f"Greet the caller by name '{name.split()[0]}'. "
            f"Ask how you can help them with bus booking today."
        )
    else:
        greeting = (
            "Greet the caller warmly in English. Introduce yourself as Neha from MavenTech. "
            "Ask how you can help them with bus booking."
        )
